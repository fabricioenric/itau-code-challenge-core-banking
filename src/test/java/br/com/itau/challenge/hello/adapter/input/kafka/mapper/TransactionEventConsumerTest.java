...
class TransactionEventConsumerTest {
    ...
    void deveProcessarEConfirmarQuandoMensagemForValida() {
        var dto = mock(br.com.itau.challenge.hello.adapter.input.kafka.dto.TransactionEventDTO.class);
        when(objectMapper.readValue(anyString(), eq(
                br.com.itau.challenge.hello.adapter.input.kafka.dto.TransactionEventDTO.class))).thenReturn(dto);

        try (var mapperMock = mockStatic(
                br.com.itau.challenge.hello.adapter.input.kafka.mapper.TransactionEventMapper.class)) {
            var event = mock(br.com.itau.challenge.hello.domain.model.TransactionEvent.class);
            mapperMock.when(() -> br.com.itau.challenge.hello.adapter.input.kafka.mapper.TransactionEventMapper
    ...
    void devePublicarNaDltEConfirmarQuandoFalharDesserializacao() {
        when(objectMapper.readValue(anyString(), eq(
                br.com.itau.challenge.hello.adapter.input.kafka.dto.TransactionEventDTO.class)))
